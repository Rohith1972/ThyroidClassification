...
def feature_classes(feature):
    ...
    df = df.fillna(0)
    df['Calcs1'] = df['Calcs1'].replace(0, "None")
...
def train_data():
    ...
    df_cancer = df[["ID", "Cancer"]].copy()
    df_compos = pd.concat([df.ID, pd.get_dummies(df.Composition)], axis=1).copy()
    df_echo = pd.concat([df.ID, pd.get_dummies(df.Echogenicity)], axis=1).copy()
    df_shape = df[["ID", "Shape"]].copy()
    df_shape["Shape"] = df_shape.apply(lambda row: 1 if row.Shape == "y" else 0, axis=1)
    df_calcs = pd.concat([df.ID, pd.get_dummies(df.Calcs1)], axis=1).copy()
    df_margin = pd.concat([df.ID, pd.get_dummies(df.MargA)], axis=1).copy()
    ...
    for f_path in train_files:
        ...
        # Skip if ID not found in dataframe
        if pid not in df_cancer['ID'].values:
            continue
            
        y_train_cancer.append(
            df_cancer[df_cancer.ID == pid].values.flatten()[1:].astype(np.float32)
        )
        y_train_compos.append(
            df_compos[df_compos.ID == pid].values.flatten()[1:].astype(np.float32)
        )
        y_train_echo.append(
            df_echo[df_echo.ID == pid].values.flatten()[1:].astype(np.float32)
        )
        # for shape feature, only assign positive label to transversal view
        if "trans" in f_path:
            shape_val = df_shape[df_shape.ID == pid].values.flatten()[1]
            y_train_shape.append(np.array([shape_val]).astype(np.float32))
        else:
            y_train_shape.append(np.array([0]).astype(np.float32))
        y_train_calcs.append(
            df_calcs[df_calcs.ID == pid].values.flatten()[1:].astype(np.float32)
        )
        y_train_margin.append(
            df_margin[df_margin.ID == pid].values.flatten()[1:].astype(np.float32)
        )
...
def test_data():
    ...
    for f_path in test_files:
        ...
        if "trans" in f_path:
            shape_val = df_shape[df_shape.ID == pid].values.flatten()[1]
            y_test_shape.append(np.array([shape_val]).astype(np.float32))
        else:
...
def validation_ids(fold, df_cancer):
    ...
    for pid in sorted(pid_set):
        label = df_cancer[df_cancer.ID == pid].values.flatten()[1:]
        if label == 1:
...
def fname2pid(fname):
    ...
    # Remove extension and view suffix (_trans, _long)
